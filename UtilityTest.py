# ****************************** Utility Functions ************************************#
#  Purpose: Solving Functional Problems,Algorithms Problem , DataStructure Problem
#  *
...
class TestFunctional:
    ...
    """ ==== replacing <<UserName>> by user input name ===== """

    @staticmethod
    def username(name):  # it is static function which accepts string as parameter
        temp = "Hello <<UserName>>, How Are You ?"  # taking the user inputs
        res = temp.replace("<<UserName>>", name)  # replace <<UserName>> by input name
        print(res)  # resulting the string username replace with input name

    """ ====  flip a coin to check head and tail percentages  ====="""

    @staticmethod
    def flipcoin(n):  # it is static function which accepts integer parameter
        head = 0  # initialize to 0
        tail = 0  # initialize to 0
        headspercentage = 0  # initialize to 0
        tailspercentage = 0  # initialize to 0

        for i in range(n):  # ranges are taken by user input
            coin = random.randint(1, 2)  # using random mathods ,randint(start,end) takes integer values
            if coin == 1:  # initialize to 1
                head += 1  # if flip to 1 head will increments
            else:
                tail += 1  # else tail will increments
        print(head)  # printing head values
        print(tail)  # printing tail values
        headspercentage = head / 10.0  # getting the heads percentage by dividing by 10
        tailspercentage = 100.0 - headspercentage  # 100.0 - heads percentage gives the tails percentage

        print("head percentages are==> " + str(headspercentage))  # to concat converting int to string
        print("tail percentages are==> " + str(tailspercentage))  # printing  heads and tails percentages
    ...
    @staticmethod
    def power2(n):  # it is a static function which accepts int as input
        i = 0
        while i <= n:  # while runs un-till loops returns false
            print(i, "*", pow(2, i))  # pow(2,i)
            i += 1  # every time i should increment by 1
    ...
    def coupon_generator(
            no_of_coupons):  # it is a static method which accepts int values numbers of coupons to generate
        my_list = list()  # using the list to hold coupons
